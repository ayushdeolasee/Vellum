...
### Step 3: Manual streaming smoke test (required — codex-cli computer-use)

Per repo guidelines, macOS UI changes must be verified with codex-cli computer-use, not skipped as optional. Run the app, open any PDF, and stream a long AI reply (any configured provider), driving and observing it via codex computer-use. Confirm: text streams smoothly, the finished message renders markdown correctly (headings, code blocks, lists), toggling system light/dark mode restyles already-rendered messages, and selecting text + quote still works. Record the verification result (what was observed, pass/fail) in your report.
...
## Addendum (2026-07-14, verified at commit `c874e13`) — drift check result: PROCEED

`SelectableMessageText.swift` changed substantially since `314cf9f` (commit `112f1c1` added markdown + LaTeX rendering), so this plan's drift check will flag it. The re-audit verified the details so the executor does not have to STOP:

1. **The target function is unchanged.** `updateNSView` at `SelectableMessageText.swift:31-49` still matches this plan's "Current state" excerpt verbatim (unconditional `AiAttributedRenderer.attributedString` call, output-string comparison afterward). `MessageContainerView.setAttributed` is at line 122 with the expected signature, and `appliedColor`/`appliedSecondary` are at lines 119-120. Steps 1–2 apply as written.
2. **The purity assumption (STOP condition #2) was re-verified and HOLDS.** `AiAttributedRenderer` was rewritten to parse via `MarkdownParser.parse` and to typeset math spans through the new `MathRenderer` (`MathRenderer.swift`). `MathRenderer.render` uses an `NSCache` keyed by `(display-mode, fontSize, resolved sRGB color, latex)` — pure memoization, deterministic output — and `MarkdownParser.parse` / `AttributedString(markdown:)` are pure. So `attributedString(for:color:secondary:)` is still a pure function of its inputs and the input-comparison optimization remains sound. (`NSColor.linkColor` inside is a dynamic system color that resolves at draw time, so it needs no cache key.)
3. **This plan now also fixes a small correctness bug, not just perf.** Math spans render as `NSTextAttachment`s, whose contribution to `textStorage.string` is a single U+FFFC placeholder regardless of the LaTeX inside. The existing output-string comparison (`storage.string != attributed.string`) therefore reports "unchanged" for an update whose only difference is inside a math span (same block structure, same prose, different equation) — the stale equation stays on screen. The input comparison (`appliedContent != content`) this plan installs catches it. Worth one extra test if convenient: apply content `"$a$"` then `"$b$"` and assert the storage changes.
4. `Tests/SelectableMessageTests.swift` is unchanged since `314cf9f`; the test-plan section stands. Dependency on plan 001 (compiling test target) stands.
5. Interaction with the newer plans from the 2026-07-14 round: plan 009 (unclosed-math gating) edits `MarkdownParser` in `MarkdownMessage.swift` — no file overlap with this plan; land in any order.
