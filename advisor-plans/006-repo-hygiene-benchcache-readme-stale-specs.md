...
>
> **Drift check (run first)**: `git diff --stat 314cf9f..HEAD -- .gitignore README.md specs/ PORT-DESIGN.md Benchmarks/README.md`
...
- Stage only the in-scope files; never `*.pbxproj`.
...
**Verify**: `xcodebuild -project Vellum.xcodeproj -scheme Vellum -configuration Debug build` → BUILD SUCCEEDED (docs/gitignore changes can't break it; this is a tripwire for accidental file damage).
