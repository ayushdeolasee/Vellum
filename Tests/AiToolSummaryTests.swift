...
// Bubble/answer width is NOT covered here. It lives with the layout it
// belongs to, in `SelectableMessageTests` and `AiPanelBubbleWidthTests`, which
// measure a real hosted `NSHostingView` rather than a width-clamping helper.
struct AiToolSummaryTests {
    ...
    // "The answer is not mixed with its tool summaries" is asserted in
    // `AiPipelineTests.testAssistantAnswerIsTheTrimmedReplyAndNothingElse`,
    // against `AiStore.assistantAnswerText`. It is deliberately not repeated
    // here: once the receipts stopped being an argument to that function, a
    // version of this test that passed a summary in and checked it did not
    // come out could no longer fail for any implementation.
