...
- `loadSettings()`: after building `settings` from the UserDefaults JSON, overwrite the key fields from the Keychain: `settings.apiKey = KeychainStore.read(account: "gemini-api-key") ?? ""` (same for openai). Then the **migration**: if the UserDefaults JSON contained a non-empty `apiKey`/`openaiApiKey` and the Keychain had none, write the UserDefaults value into the Keychain and adopt it into `settings`. Finally — and independently of which branch ran — if the UserDefaults JSON contained **any** non-empty key material, immediately call `saveSettings(settings)` so the plaintext copy is purged (see next bullet). The Keychain stays authoritative when both stores hold a value (e.g. after an interrupted earlier migration); the point is that a surviving plaintext copy must be rewritten out in that case too, not only when the Keychain was empty.
