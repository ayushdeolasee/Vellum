...
struct AiToolSummary: Codable, Equatable, Identifiable, Sendable {
    ...
    /// Fold one executed tool call into the trace shown under the reply.
    ///
    /// The caps come from `AiPersistence` rather than being restated as
    /// literals here. Everything this returns is re-bounded by
    /// `AiPersistence.sanitizeToolSummaries` before it is written, so a local
    /// copy of the numbers would only ever be able to drift below the real
    /// limit and silently over-truncate — which is exactly the duplication
    /// `VellumBundle.capConversation` used to be, and which this PR deletes.
    static func make(action: AiToolAction, result: String) -> AiToolSummary? {
        ...
        case "searchDocument":
            ...
            // With no parsed hits the raw result is one of the engine's own
            // short status lines ("No matches for …", "Skipped searchDocument:
            // …"), which is genuinely the most useful thing to show. It is NOT
            // a page payload: every payload line parses into a `Source`, so a
            // result with content to leak never reaches this branch.
            let detail = count == 0
                ? firstLine(of: result)
                : "\(count) \(count == 1 ? "page" : "pages")"
            return AiToolSummary(
                title: clipped("Searched for “\(query)”", limit: titleLimit),
                detail: clipped(detail, limit: detailLimit),
                sources: Array(sources.prefix(sourceLimit))
            )
        ...
        case "getPageText":
            ...
            let body = result
                ...
                .first
                .map(String.init) ?? ""
            let source = Source(page: page, excerpt: clipped(body, limit: excerptLimit))
            return AiToolSummary(
        ...
        case "getAnnotations":
            // Only the engine's count header ("3 annotations in this
            // document:"), never the lines under it — those carry the user's
            // highlighted text and note bodies verbatim, and raw tool payload
            // must not land in the visible transcript. The model still gets
            // the full list; this is the display copy.
            return AiToolSummary(
                title: "Read annotations",
                detail: clipped(firstLine(of: result), limit: detailLimit)
            )

        default:
            // Write tools (goToPage / addNote / addHighlight) and the unknown
            // tool fallback. Their results are already short one-line receipts
            // authored for display, so the result IS the title.
            let page = pageNumber(from: result) ?? roundedPage(action.args.pageNumber)
            return AiToolSummary(
                title: clipped(result, limit: titleLimit),
    ...
    /// Leading line of a tool result, with the trailing ":" of the engine's
    /// list headers dropped so it reads as a phrase rather than a stub.
    private static func firstLine(of result: String) -> String {
        let line = result
            .split(separator: "\n", maxSplits: 1, omittingEmptySubsequences: true)
            .first
            .map(String.init) ?? ""
    ...
    private static var titleLimit: Int { AiPersistence.maxToolSummaryTitleCharacters }
    private static var detailLimit: Int { AiPersistence.maxToolSummaryDetailCharacters }
    private static var excerptLimit: Int { AiPersistence.maxToolSourceExcerptCharacters }
    private static var sourceLimit: Int { AiPersistence.maxToolSourcesPerSummary }
    ...
    private static func legacyCompactSummary(_ line: String) -> AiToolSummary? {
        ...
        if fullMatch(receipt, pattern: #"Searched the document for “.+”\."#) {
            return AiToolSummary(title: clipped(receipt, limit: titleLimit), detail: "Document search")
        }
        ...
        return AiToolSummary(
            title: clipped(receipt, limit: titleLimit),
            detail: "Document action",
    ...
    private static func searchSources(from result: String) -> [Source] {
        result.split(separator: "\n").compactMap { rawLine in
            ...
            return Source(page: page, excerpt: clipped(excerpt, limit: excerptLimit))
        }
    }
