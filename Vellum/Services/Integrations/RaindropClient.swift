...
struct RaindropClient: RaindropServing, Sendable {
    ...
    func validate(token: String) async throws { var request = URLRequest(url: baseURL.appending(path: "rest/v1/user")); request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization"); _ = try await http.perform(request, provider: .raindrop, idempotent: true) }
    func collections(token: String) async throws -> [ReadLaterCollection] {
    ...
    func page(token: String, page: Int, perPage: Int = 50) async throws -> IntegrationPage {
        ...
        return try Self.mapPage((try await http.perform(request, provider: .raindrop, idempotent: true)).data, page: page, perPage: perPage)
    }
    func moveItem(token: String, itemID: String, collectionVendorID: String) async throws {
        ...
        let data = (try await http.perform(request, provider: .raindrop, idempotent: true)).data
    ...
    /// Provider-supplied item ids are interpolated into the URL path above; reject anything
    /// outside the charset Raindrop actually emits so a hostile id can't retarget the request
    /// via `URL.appending(path:)`'s "/" handling.
    static func isValidVendorID(_ value: String) -> Bool {
        !value.isEmpty && value.allSatisfy { $0.isASCII && ($0.isLetter || $0.isNumber || $0 == "-" || $0 == "_") }
    }
    private func collectionRequest(token: String, path: String) async throws -> [RaindropCollectionDTO] { var request = URLRequest(url: baseURL.appending(path: path)); request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization"); let object = try JSONSerialization.jsonObject(with: (try await http.perform(request, provider: .raindrop, idempotent: true)).data) as? [String: Any]; guard let values = object?["items"] else { throw IntegrationError.malformedData }; return try JSONDecoder().decode([RaindropCollectionDTO].self, from: JSONSerialization.data(withJSONObject: values)) }
    ...
    static func flattenCollections(_ values: [RaindropCollectionDTO]) -> [ReadLaterCollection] {
        ...
        func append(_ parent: String?, _ depth: Int) { for child in (grouped[parent] ?? []).sorted(by: { $0.sort == $1.sort ? $0.title.localizedStandardCompare($1.title) == .orderedAscending : $0.sort > $1.sort }) where visited.insert(child.id.stringValue).inserted { result.append(.init(provider: .raindrop, vendorID: child.id.stringValue, title: child.title, parentID: child.parentID.map { ReadLaterCollection.id(provider: .raindrop, vendorID: $0.stringValue) }, depth: depth, sortIndex: result.count)); append(child.id.stringValue, depth + 1) } }
