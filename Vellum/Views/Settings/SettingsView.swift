...
/// Manages the on-disk extracted-text cache (`PageTextCache`) and downloaded
/// web pages: total sizes, per-item breakdowns sorted by size, destructive
/// per-item and bulk controls, the auto-save-every-page toggle, and the
/// storage-location picker (iCloud / custom folder / this iPad). Unlike its
/// sibling tabs this one scrolls — the lists can grow unbounded — so it does
/// NOT set `.scrollDisabled(true)`.
private struct StorageSettingsTab: View {
    ...
    private var locationFooterText: String {
        ...
        switch storageMode {
        case .icloud:
            return "Everything — offline copies, highlights, notes, and reading positions — lives in iCloud Drive ▸ Vellum and syncs across your devices."
        case .custom:
            return "Offline copies live in your folder. iCloud syncing is not available for a custom folder: highlights, notes, and reading positions stay on this iPad."
        case .local:
    ...
    private var locationBinding: Binding<WebStorageMode> {
        Binding(
            ...
            set: { newMode in
                ...
                switch newMode {
                case .custom:
                    // The folder picker is asynchronous; its callback applies
                    // the change and refreshes. Cancelling leaves the mode as-is.
                    WebStorageRelocator.chooseCustomFolder {
                        refreshStorageSettings()
                        Task {
                            try? await Task.sleep(for: .seconds(1))
